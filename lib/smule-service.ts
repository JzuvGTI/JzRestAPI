...
const SOWNLOADER_BASE_URL = "https://sownloader.com";
const SOWNLOADER_ENDPOINT = `${SOWNLOADER_BASE_URL}/index.php`;

type SmuleDownloadResult = {
  metadata: {
    title: string;
    smule_link: string | null;
...
function toAbsoluteUrl(value: string | undefined | null) {
  ...
  if (trimmed.startsWith("/")) {
    return `${SOWNLOADER_BASE_URL}${trimmed}`;
  }

  return trimmed;
}

function isValidSmuleUrl(value: string) {
  try {
    const url = new URL(value);
    return url.hostname === "smule.com" || url.hostname.endsWith(".smule.com");
  } catch {
    return false;
  }
}

function extractMp3Url(onclickValue: string | undefined, title: string) {
  if (!onclickValue) {
    return null;
  }

  const match = onclickValue.match(/['"](https?:\/\/[^'"]+\.m4a[^'"]*)['"]/i);
  if (!match) {
    return null;
  }

  return `${SOWNLOADER_BASE_URL}/system/modules/downloader.php?url=${encodeURIComponent(
    match[1],
  )}&name=${encodeURIComponent(title || "smule-audio")}&ext=mp3`;
}

async function fetchSmuleDownloadData(smuleUrl: string): Promise<SmuleDownloadResult> {
  const response = await axios.get(SOWNLOADER_ENDPOINT, {
    timeout: 30000,
    params: {
      url: smuleUrl,
    },
  ...
  const title = $("h4 a").first().text().trim() || "Untitled";
  const smuleLink = toAbsoluteUrl($("h4 a").first().attr("href"));
  const thumbnail = toAbsoluteUrl($(".sownloader-web-thumbnail").first().attr("src"));
  const description = $(".sownloader-web-thumbnail").closest(".row").find("p").first().text().trim() || "-";

  const audioM4a = toAbsoluteUrl($('a.btn[href*=".m4a"]').first().attr("href"));
  const videoMp4 = toAbsoluteUrl($('a.btn[href*=".mp4"]').first().attr("href"));

  const mp3ButtonOnclick =
    $("button.btn")
      .filter((_, el) => $(el).text().toLowerCase().includes("download as mp3"))
      .first()
      .attr("onclick") || undefined;
  const audioMp3 = extractMp3Url(mp3ButtonOnclick, title);
...
export async function handleSmuleDownloadRequest(url: URL) {
  const endpointError = await checkEndpointAvailability("smule-dl");
  if (endpointError) {
    return endpointError;
  }

  const smuleUrl = (url.searchParams.get("url") || "").trim();
  const apiKeyValue = (url.searchParams.get("apikey") || "").trim();

  if (!smuleUrl) {
    return errorResponse(400, "Query parameter 'url' is required.");
  }
  if (!isValidSmuleUrl(smuleUrl)) {
    return errorResponse(400, "Query parameter 'url' must be a valid Smule URL.");
  }
  if (!apiKeyValue) {
  ...
  try {
    const result = await fetchSmuleDownloadData(smuleUrl);

    if (!result.audio.m4a && !result.audio.mp3 && !result.video.mp4) {
